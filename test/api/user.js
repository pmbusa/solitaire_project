...
describe('User:', () => {
    ...
    before(done => {
        harness.setup(config.mongoURL, done);
    });
    after(done => {
    ...
    describe('Create:', () => {
        it('Failure - missing required username', done => {
            primaryAgent.post(`${config.url}user`)
        ...
        it('Failure - missing required address', done => {
            primaryAgent.post(`${config.url}user`)
                .send(_.pick(users.primary, 'first_name', 'last_name', 'username', 'password'))
                .end((err, res) => {
                    res.status.should.equal(400);
        ...
        it('Failure - missing required password', done => {
            primaryAgent.post(`${config.url}user`)
                .send(_.pick(users.primary, 'first_name', 'last_name', 'username', 'primary_email'))
                .end((err, res) => {
                    res.status.should.equal(400);
        ...
        it('Failure - malformed username -- bad chars', done => {
            ...
            primaryAgent.post(`${config.url}user`)
                ...
                .end((err, res) => {
                    res.status.should.equal(400);
                    res.body.error.should.equal('"username" must only contain alpha-numeric characters');
                    done();
                });
        });
        ...
        it('Failure - malformed password -- too short', done => {
            ...
            primaryAgent.post(`${config.url}user`)
                ...
                .end((err, res) => {
                    res.status.should.equal(400);
                    res.body.error.should.equal('"password" length must be at least 8 characters long');
                    done();
                });
        });
        it('Failure - malformed password -- need at least one uppercase', done => {
            ...
            primaryAgent.post(`${config.url}user`)
                ...
                .end((err, res) => {
                    res.status.should.equal(400);
                    res.body.error.should.equal('"password" must contain at least one uppercase character');
                    done();
                });
        });
        it('Failure - malformed password -- need at least one lowercase', done => {
            ...
            primaryAgent.post(`${config.url}user`)
                ...
                .end((err, res) => {
                    res.status.should.equal(400);
                    res.body.error.should.equal('"password" must contain at least one lowercase character');
                    done();
                });
        });
        it('Failure - malformed password -- need at least one number', done => {
            ...
            primaryAgent.post(`${config.url}user`)
                ...
                .end((err, res) => {
                    res.status.should.equal(400);
                    res.body.error.should.equal('"password" must contain at least one numeric character');
                    done();
                });
        });
        it('Failure - malformed password -- need at least one symbol', done => {
            ...
            primaryAgent.post(`${config.url}user`)
                ...
                .end((err, res) => {
                    res.status.should.equal(400);
                    res.body.error.should.equal('"password" must contain at least one of: @, !, #, $, % or ^');
                    done();
                });
        });
    ...
    describe('Exists:', () => {
        it('Failure - unknown user', done => {
            primaryAgent.head(`${config.url}user/fakeusername`)
                .end((err, res) => {
        ...
        it('Success - read existing user profile', done => {
            primaryAgent.head(`${config.url}user/${users.primary.username}`)
                .end((err, res) => {
        ...
        it('Success - read existing user profile - case insensitive', done => {
            primaryAgent.head(`${config.url}user/${users.primary.username.toUpperCase()}`)
                .end((err, res) => {
                    res.status.should.equal(200);
                    done();
                });
    ...
    describe('Read:', () => {
        it('Success - read existing user profile', done => {
            primaryAgent.get(`${config.url}user/${users.primary.username}`)
                .end((err, res) => {
                    res.status.should.equal(200);
                    res.body.username.should.equal(users.primary.username);
                    res.body.primary_email.should.equal(users.primary.primary_email);
        ...
        it('Failure - read non-existent user profile', done => {
            primaryAgent.get(`${config.url}user/foobar`)
                .end((err, res) => {
                    res.status.should.equal(404);
    ...
    describe('Update:', () => {
        ...
        it('Success - update first and last name - return 204', done => {
            primaryAgent.put(`${config.url}user`)
        ...
        it('Success - can not change username, primary_email or non-standard fields - return 204', done => {
            primaryAgent.put(`${config.url}user`)
                .send({ username: 'newname', primary_email: 'something@new.com', admin: true })
                .end((err, res) => {
                    res.status.should.equal(204);
                    done();
                });
